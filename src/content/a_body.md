...
If a translator uses Microsoft Word, a globally recognized word processor, the proofing tool is activated by default, regardless of whether the Chinese or English version is used. This is likely the most common orthography check tool for an inexperienced translator. However, many translated texts fail to pass Microsoft Word's proofing. For example, Microsoft Word (version 2404) detected errors in the sentence: `Animals and Plants Introduced by Zhang Qian From the Western Regions.` with `From` being capitalized uncorectly.
